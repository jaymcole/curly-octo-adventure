...
import curly.octo.server.serverAgents.NPCBehaviorAgent;
...
public class GameServer {
    ...
    private final NPCElectionManager npcElectionManager;
    ...
    // NPC position tracking (for completion detection and squishy collision)
    private final HashMap<String, com.badlogic.gdx.math.Vector3> npcPositions = new HashMap<>();
    private final HashMap<String, Long> npcLastUpdateTime = new HashMap<>();
    private NPCBehaviorAgent npcBehaviorAgent;  // Reference for completion-based instruction generation
    ...
    public GameServer(Random random, ServerGameObjectManager gameObjectManager, ServerCoordinator serverCoordinator) {
        ...
        this.npcElectionManager = new NPCElectionManager(serverCoordinator.clientManager);
    ...
    /**
     * Handle NPC sync message from elected client and relay to all other clients.
     ...
     *
     * @param agent The NPC behavior agent
     */
     ...
     */
    public com.badlogic.gdx.math.Vector3 getNPCPosition(String npcId) {
    ...
    public void regenerateMap(long newSeed, String reason, boolean isInitialGeneration) {
        ...
        regenerationThread = new Thread(() -> {
            try {
                ...
                if (currentMap != null) {
                    ArrayList<MapHint> spawnHints = currentMap.getAllHintsOfType(SpawnPointHint.class);
                    if (!spawnHints.isEmpty()) {
                        int spawnIndex = 0;
                        for (WalkingCharacter player : gameObjectManager.getAllPlayers()) {
...
//                        serverCoordinator.removePlayerFromEnvironment(disconnectedPlayer);
    ...
    public String getPlayerIdForConnection(int connectionId) {
