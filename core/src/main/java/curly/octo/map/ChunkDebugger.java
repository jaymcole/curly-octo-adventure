...
public class ChunkDebugger {
    ...
    private static void debugRawMapData(GameMap gameMap) {
        ...
        int minTileX = (int)(minX / MapTile.TILE_SIZE);
        int maxTileX = (int)(maxX / MapTile.TILE_SIZE);
        int minTileY = (int)(minY / MapTile.TILE_SIZE);
        int maxTileY = (int)(maxY / MapTile.TILE_SIZE);
        int minTileZ = (int)(minZ / MapTile.TILE_SIZE);
        int maxTileZ = (int)(maxZ / MapTile.TILE_SIZE);
        ...
        for (int i = 0; i < Math.min(5, allTiles.size()); i++) {
            ...
            Vector3 tileCoords = new Vector3(
                (int)(tile.x / MapTile.TILE_SIZE),
                (int)(tile.y / MapTile.TILE_SIZE),
                (int)(tile.z / MapTile.TILE_SIZE)
            );
            Log.info("ChunkDebugger", String.format(
